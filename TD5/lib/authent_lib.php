<?php
  ...
  */
  function check_login_with_session(callable $authent_function, ?string $session_name = NULL) : ?Identite {
    ...
    if (isset($_SESSION['ident']) && $_SESSION['ident'] instanceof Identite) {
        return $_SESSION['ident'];
    }

    try {
        // Tentez de vérifier les informations d'identification
        $user = check_login_post($authent_function);
        $_SESSION['ident'] = $user; // Stockez l'identité dans la session
        return $user;
    } catch (AuthentException $e) {
        throw $e; // Propager l'exception en cas d'échec
    }
}
